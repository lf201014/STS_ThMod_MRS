...
public class GalacticHaloPower
	...
	public void onPlayCard(AbstractCard card, AbstractMonster m) {
		if (card.type == CardType.ATTACK) {
			this.cnt++;
			ThMod.logger.info("GalacticHaloPower : Adding counter for playing :"+card.cardID+" ; counter :"+this.cnt);
		}
	}
	
	public void atEndOfTurn(boolean isPlayer) {
		AbstractPlayer p = AbstractDungeon.player;
		ThMod.logger.info("GalacticHaloPower : end of turn : counter : "+this.cnt+" ; Player turn :"+isPlayer);
		if ((isPlayer)&&(this.cnt > 0)) {
			ThMod.logger.info("GalacticHaloPower : Granting block ; counter : "+cnt+" ; amount :"+this.amount);
