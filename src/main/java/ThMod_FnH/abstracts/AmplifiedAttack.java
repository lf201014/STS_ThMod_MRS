...
public abstract class AmplifiedAttack extends CustomCard {
  ...
  public void applyPowers() {
    ...
    if (!this.isMultiDamage) {
      ...
      ApplyDamageGivePower(tmp, player);
      ApplyDamageGivePower(amp, player);
      ApplyDamageFinalGivePower(tmp, player);
      ApplyDamageFinalGivePower(amp, player);
      if (tmp < 0.0F) {
        tmp = 0.0F;
      }
      if (amp < 0.0F) {
      ...
      for (int i = 0; i < tmp.length; i++) {
        ...
        ApplyDamageGivePower(tmp[i], player);
        ApplyDamageGivePower(amp[i], player);
        ApplyDamageFinalGivePower(tmp[i], player);
        ApplyDamageFinalGivePower(amp[i], player);
      }
  ...
  public void calculateCardDamage(AbstractMonster mo) {
    ...
    if ((!this.isMultiDamage) && (mo != null)) {
      ...
      ApplyDamageGivePower(tmp, player);
      ApplyDamageGivePower(amp, player);
      ApplyDamageReceivePower(tmp, mo);
      ApplyDamageReceivePower(amp, mo);

      ApplyDamageFinalGivePower(tmp, player);
      ApplyDamageFinalGivePower(amp, player);
      ApplyDamageFinalReceivePower(tmp, mo);
      ApplyDamageFinalReceivePower(amp, mo);
      ...
      for (int i = 0; i < tmp.length; i++) {
        ...
        ApplyDamageGivePower(tmp[i], player);
        ApplyDamageGivePower(amp[i], player);
        for (AbstractMonster mon : m) {
          ApplyDamageReceivePower(tmp[i], mon);
          ApplyDamageReceivePower(amp[i], mon);
        }
        ApplyDamageFinalGivePower(tmp[i], player);
        ApplyDamageFinalGivePower(amp[i], player);
        for (AbstractMonster mon : m) {
          ApplyDamageFinalReceivePower(tmp[i], mon);
          ApplyDamageFinalReceivePower(amp[i], mon);
        }
      }
      DamageModifiedCheck(tmp, amp);
      this.multiDamage = new int[tmp.length];
  ...
  private void ApplyDamageGivePower(float damage, AbstractCreature source) {
    for (AbstractPower p : source.powers) {
      damage = p.atDamageGive(damage, this.damageTypeForTurn);
    }
  }

  private void ApplyDamageFinalGivePower(float damage, AbstractCreature source) {
    for (AbstractPower p : source.powers) {
      damage = p.atDamageFinalGive(damage, this.damageTypeForTurn);
    }
  }

  private void ApplyDamageReceivePower(float damage, AbstractCreature target) {
    if ((!(target.isDying)) && (!(target.isEscaping))) {
      for (AbstractPower p : target.powers) {
        damage = p.atDamageReceive(damage, this.damageTypeForTurn);
      }
    }
  }

  private void ApplyDamageFinalReceivePower(float damage, AbstractCreature target) {
    if ((!(target.isDying)) && (!(target.isEscaping))) {
      for (AbstractPower p : target.powers) {
        damage = p.atDamageFinalReceive(damage, this.damageTypeForTurn);
      }
    }
  }

  private void DamageModifiedCheck(float[] tmp, float[] amp) {
    for (int i = 0; i < tmp.length; i++) {
      if (tmp[i] < 0.0F) {
        tmp[i] = 0.0F;
      }
